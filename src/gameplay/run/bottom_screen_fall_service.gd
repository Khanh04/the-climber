class_name BottomScreenFallService
extends RefCounted
...
func resolve(controller: RefCounted, player: Node, run_session: RefCounted) -> void:
	Validation.require_condition(controller != null, "BottomScreenFallService requires a climb controller.")
	Validation.require_condition(controller is ClimbPrototypeControllerScript, "BottomScreenFallService requires a ClimbPrototypeController implementation.")
	Validation.require_condition(player != null, "BottomScreenFallService requires a player character.")
	Validation.require_condition(player is PlayerCharacterScript, "BottomScreenFallService requires a PlayerCharacter implementation.")
	Validation.require_condition(run_session != null, "BottomScreenFallService requires a run session.")
	Validation.require_condition(run_session is RunSessionScript, "BottomScreenFallService requires a RunSession implementation.")
	...
	typed_run_session.resolve_fall(RunEndReasonScript.Value.BOTTOM_SCREEN_FALL)