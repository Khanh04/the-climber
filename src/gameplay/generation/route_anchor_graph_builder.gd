...
func _init(chunk_width_meters_value: float, row_step_height_meters_value: float) -> void:
...
func _build_local_position(row_index: int, lane: int) -> Vector2:
	...
	return Vector2(half_width_meters * lane_offset, -row_step_height_meters * float(row_index + 1))
