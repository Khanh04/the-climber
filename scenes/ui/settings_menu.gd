...
@onready var _audio_slider: HSlider = $CenterContainer/Panel/ControlPosition/AudioControl/AudioSlider
@onready var _audio_value_label: Label = $CenterContainer/Panel/ControlPosition/AudioControl/value_audio
...
func _refresh_value_labels() -> void:
	_audio_value_label.text = "%d%%" % roundi(_audio_slider.value)
	_audio_slider.tooltip_text = "Nhạc nền: " + _audio_value_label.text
	_volume_value_label.text = "%d%%" % roundi(_volume_slider.value)
	_touch_split_value_label.text = "%d%%" % roundi(lerpf(35.0, 65.0, _touch_split_slider.value / 100.0))
	_touch_dead_zone_value_label.text = "%d%%" % roundi(_touch_dead_zone_slider.value * 0.2)

	_volume_slider.tooltip_text = "Âm lượng: " + _volume_value_label.text
	_touch_split_slider.tooltip_text = "Vùng trái: " + _touch_split_value_label.text
	_touch_dead_zone_slider.tooltip_text = "Vùng giữa không nhận chạm: " + _touch_dead_zone_value_label.text
...
func _on_touch_split_value_changed(value: float) -> void:
	...
	var ratio: float = lerpf(0.35, 0.65, value / 100.0)
...
func _on_touch_dead_zone_value_changed(value: float) -> void:
	...
	var ratio: float = lerpf(0.0, 0.20, value / 100.0)
...
func _validate_required_nodes() -> void:
	...
	Validation.require_condition(_audio_slider != null, "SettingsMenu requires AudioSlider.")
	Validation.require_condition(_audio_value_label != null, "SettingsMenu requires value_audio.")
