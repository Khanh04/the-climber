...
func test_candidate_selection_is_not_stuck_on_the_first_attempt_across_seeds() -> void:
    # Pre-A7 the selector kept the least-committing candidate, which was almost
    # always attempt 0. The difficulty-target metric plus seeded tie-break must
    # now spread the choice across attempts for the same baseline-band chunk.
    var chosen_attempt_indices: Dictionary[int, bool] = {}
    for day in range(1, 17):
        var tuning: GenerationTuningScript = GenerationTuningScript.new()
        var generator: DailyChunkGeneratorScript = DailyChunkGeneratorScript.new(tuning)
        var seed_key: String = DailySeedKey.from_utc_date(2026, 5, day)
        var layout: GeneratedChunkLayoutScript = _require_chunk_layout(generator.build_chunk(seed_key, 6))
        assert_eq(layout.difficulty_band, ChunkDifficultyBandScript.Value.BASELINE)
        chosen_attempt_indices[layout.selected_candidate_attempt_index] = true
